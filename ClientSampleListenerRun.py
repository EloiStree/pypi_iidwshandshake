...
async def handle_byte_received(bytes_received):
    ...
    print (f"Received from server: {bytes_received}")
...
braodcast_port = [7000,3615]
byte_queue = list()
def loop_queue_push():
    while True:
        while len(byte_queue)>0:
            bytes_to_send = byte_queue.pop(0)
            for port in braodcast_port:
                sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                sock.sendto(bytes_to_send, ('127.0.0.1', port))
                sock.close()
